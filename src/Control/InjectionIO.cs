...
public static class InjectionIO
{
    ...
    // A file that names what it brings needs a build that reads names. One that
    // states slots alone reads on every build that ever read it.
    private const uint _iteration = 11;
    private const uint _symbolIteration = 12;
    ...
    public static byte[] Serialize(InjectionData data)
    {
        ...
        finalWriter.Write(data.Symbols.Count > 0 ? _symbolIteration : _iteration);
    ...
    private static int WriteEdits(InjectionData data, TRLevelWriter writer)
    {
        ...
        blockCount += WriteBlock(BlockType.AnimEdits, data.AnimEdits.Count, writer,
            s => data.AnimEdits.ForEach(a => a.Serialize(s, data.GameVersion)));

        blockCount += WriteBlock(BlockType.ObjectLinkEdits, data.ObjectLinkEdits.Count, writer,
            s => data.ObjectLinkEdits.ForEach(o => o.Serialize(s, data.GameVersion)));

        blockCount += WriteBlock(BlockType.PropertyEdits, data.PropertyEdits.Count, writer,
            s => data.PropertyEdits.ForEach(o => o.Serialize(s, data.GameVersion)));
